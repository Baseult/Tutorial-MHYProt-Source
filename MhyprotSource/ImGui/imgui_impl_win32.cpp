...
#endif
IMGUI_IMPL_API LRESULT im_gui_impl_win32_wnd_proc_handler(HWND hwnd, UINT msg, WPARAM w_param, LPARAM l_param)
{
	...
	case WM_MOUSEMOVE:
		...
		io.AddMousePosEvent((float)GET_X_LPARAM(l_param), (float)GET_Y_LPARAM(l_param));
		break;
	case WM_MOUSELEAVE:
	...
	case WM_CHAR:
		...
		return 0;
	case WM_SETCURSOR:
		if (LOWORD(l_param) == HTCLIENT && ImGui_ImplWin32_UpdateMouseCursor())
			return 1;
		return 0;
	case WM_DEVICECHANGE:
		if ((UINT)w_param == DBT_DEVNODES_CHANGED)
			bd->WantUpdateHasGamepad = true;
		return 0;
	}
	return 0;
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
	if (_IsWindows10OrGreater())
	{
		...
		if (auto SetThreadDpiAwarenessContextFn = (PFN_SetThreadDpiAwarenessContext)
