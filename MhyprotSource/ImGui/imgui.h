...
using ImGuiCond = int; // -> enum ImGuiCond_            // Enum: A condition for many Set*() functions
...
using ImGuiKey = int; // -> enum ImGuiKey_             // Enum: A key identifier
using ImGuiNavInput = int; // -> enum ImGuiNavInput_        // Enum: An input identifier for navigation
using ImGuiMouseButton = int;
// -> enum ImGuiMouseButton_     // Enum: A mouse button identifier (0=left, 1=right, 2=middle)
using ImGuiMouseCursor = int; // -> enum ImGuiMouseCursor_     // Enum: A mouse cursor identifier
using ImGuiSortDirection = int; // -> enum ImGuiSortDirection_   // Enum: A sorting direction (ascending or descending)
using ImGuiStyleVar = int; // -> enum ImGuiStyleVar_        // Enum: A variable identifier for styling
using ImGuiTableBgTarget = int; // -> enum ImGuiTableBgTarget_   // Enum: A color target for TableSetBgColor()
...
using ImGuiColorEditFlags = int; // -> enum ImGuiColorEditFlags_  // Flags: for ColorEdit4(), ColorPicker4() etc.
...
using ImGuiHoveredFlags = int; // -> enum ImGuiHoveredFlags_    // Flags: for IsItemHovered(), IsWindowHovered() etc.
using ImGuiInputTextFlags = int; // -> enum ImGuiInputTextFlags_  // Flags: for InputText(), InputTextMultiline()
using ImGuiKeyModFlags = int; // -> enum ImGuiKeyModFlags_     // Flags: for io.KeyMods (Ctrl/Shift/Alt/Super)
...
using ImS64 = signed long long; // 64-bit signed integer (C++11)
using ImU64 = unsigned long long; // 64-bit unsigned integer (C++11)
...
using ImGuiInputTextCallback = int(*)(ImGuiInputTextCallbackData* data); // Callback function for ImGui::InputText()
...
using ImGuiMemFreeFunc = void(*)(void* ptr, void* user_data); // Function signature for ImGui::SetAllocatorFunctions()
...
namespace ImGui
{
	...
	IMGUI_API bool ImageButtonPadding(ImTextureID user_texture_id, const ImVec2& size,
	                                  const ImVec2& padding = ImVec2(0, 0), const ImVec2& uv0 = ImVec2(0, 0),
	                                  const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1,
...
struct ImGuiIO
{
	...
	bool KeysDown[512];
	// [LEGACY] Input: Keyboard keys that are pressed (ideally left in the "native" order your engine has access to keyboard keys, so you can use your own defines/enums for keys).
#endif
...
using ImDrawCornerFlags = ImDrawFlags;
