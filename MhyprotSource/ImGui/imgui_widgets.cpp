		...
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
		{
			...
			if (mouse_button_clicked != -1 && g.ActiveId != id)
			{
				...
				if ((flags & ImGuiButtonFlags_PressedOnClick) || ((flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.
					IO.MouseClickedCount[mouse_button_clicked] == 2))
				{
					pressed = true;
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse)
		{
			...
			else
			{
				...
				if ((release_in || release_anywhere) && !g.DragDropActive)
				{
					...
					bool is_double_click_release = (flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseReleased
						[mouse_button] && g.IO.MouseClickedLastCount[mouse_button] == 2;
                        ...
                        ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	const float grab_h_pixels = ImClamp(scrollbar_size_v * ((float)size_avail_v / (float)win_size_v), style.GrabMinSize,
	                                    scrollbar_size_v);
...
// frame_padding = 0: no framing
// frame_padding > 0: set framing size
bool ImGui::ImageButtonPadding(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& padding,
                               ...
                               const ImVec4& tint_col)
{
	...
	const ImVec2 im_padding = (frame_padding >= 0)
		                          ? ImVec2((float)frame_padding, (float)frame_padding)
		                          : g.Style.FramePadding;
	return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	auto label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
	if (g.LogEnabled)
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
	...
	if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32 || data_type == ImGuiDataType_S64 || data_type
		== ImGuiDataType_U64 || data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
	{
                       ...
                       const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	{
		const bool input_requested_by_tabbing = temp_input_allowed && (g.LastItemData.StatusFlags &
			ImGuiItemStatusFlags_FocusedByTabbing) != 0;
		const bool clicked = (hovered && g.IO.MouseClicked[0]);
		const bool double_clicked = (hovered && g.IO.MouseClickedCount[0] == 2);
		if (input_requested_by_tabbing || clicked || double_clicked || g.NavActivateId == id || g.NavActivateInputId ==
			id)
		{
			SetActiveID(id, window);
			SetFocusID(id, window);
			...
			if (temp_input_allowed)
				if (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || double_clicked || g.NavActivateInputId ==
					id)
	...
	else
	{
		...
		else
		{
			...
			if (t < 1.0)
			{
				...
				result = (TYPE)((SIGNEDTYPE)v_min + (SIGNEDTYPE)(v_new_off_f + (FLOATTYPE)(
					v_min > v_max ? -0.5 : 0.5)));
			}
			else
			{
                         ...
                         const char* format, ImGuiSliderFlags flags)
{
	...
	const ImRect frame_bb(window->DC.CursorPos,
	                      window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f + padding));
	const ImRect total_bb(frame_bb.Min,
	                      frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f,
	                                            0.0f));
	...
	if (!temp_input_is_active)
	{
		...
		if (input_requested_by_tabbing || clicked || g.NavActivateId == id || g.NavActivateInputId == id)
		{
			...
			FocusWindow(window);
			g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
			if (temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || g.NavActivateInputId
				== id))
				temp_input_is_active = true;
		}
	}
	...
	// Draw frame
	// OWN EDIT
	const ImU32 frame_col = GetColorU32(ImGuiCol_FrameBgWhite);
	//GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
	const ImU32 frame_col_active = GetColorU32(ImGuiCol_FrameBgActive);
	//GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
	RenderNavHighlight(frame_bb, id);
	...
	{
		auto size = 2;
		auto centerY = (frame_bb.Min.y + frame_bb.Max.y) / 2;
		auto localFrameMin = ImVec2(frame_bb.Min.x, centerY - size);
		auto localFrameMax = ImVec2(grab_bb.Max.x, centerY + size);

		auto localFrameMinEnd = ImVec2(grab_bb.Min.x, centerY - size);
		auto localFrameMaxEnd = ImVec2(frame_bb.Max.x, centerY + size);
		RenderFrame(localFrameMin, localFrameMax, frame_col_active, true, size * 2);
		RenderFrame(localFrameMinEnd, localFrameMaxEnd, frame_col, true, size * 2);
	}
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
	{
		auto center = ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, (grab_bb.Min.y + grab_bb.Max.y) / 2);
		window->DrawList->AddCircleFilled(center, 10.f,
		                                  GetColorU32(
			                                  g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 30);
		window->DrawList->AddCircleFilled(center, 7.f, GetColorU32(ImGuiCol_FrameBgActive), 30);
	}
	...
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
		                  frame_bb.Min.y + style.FramePadding.y + padding / 2), label);
	//RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	bool render_selection = state && (state->HasSelection() || select_all) && (RENDER_SELECTION_WHEN_INACTIVE ||
		render_cursor);
	bool value_changed = false;
	...
	if (g.ActiveId == id)
	{
		...
		else if (io.MouseDown[0] && !state->SelectedAllMouseLock && (io.MouseDelta.x != 0.0f || io.MouseDelta.y !=
			0.0f))
		{
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_cut = ((is_shortcut_key && IsKeyPressed(ImGuiKey_X)) || (is_shift_key_only &&
			IsKeyPressed(ImGuiKey_Delete))) && !is_readonly && !is_password && (!is_multiline || state->HasSelection());
		const bool is_copy = ((is_shortcut_key && IsKeyPressed(ImGuiKey_C)) || (is_ctrl_key_only &&
			IsKeyPressed(ImGuiKey_Insert))) && !is_password && (!is_multiline || state->HasSelection());
		const bool is_paste = ((is_shortcut_key && IsKeyPressed(ImGuiKey_V)) || (is_shift_key_only &&
			IsKeyPressed(ImGuiKey_Insert))) && !is_readonly;
		const bool is_undo = ((is_shortcut_key && IsKeyPressed(ImGuiKey_Z)) && !is_readonly && is_undoable);
		const bool is_redo = ((is_shortcut_key && IsKeyPressed(ImGuiKey_Y)) || (is_osx_shift_shortcut &&
			IsKeyPressed(ImGuiKey_Z))) && !is_readonly && is_undoable;
		...
		const bool is_cancel = IsKeyPressed(ImGuiKey_Escape) || IsNavInputTest(
			ImGuiNavInput_Cancel, ImGuiInputReadMode_Pressed);
	...
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			if (Selectable("##selectable", false, 0, picker_size)) // By default, Selectable() is closing popup
				g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags &
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if ((flags & ImGuiSelectableFlags_SelectOnNav) && g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == window
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
	...
	if (label_size.x > 0.0f)
	{
		auto label_pos = ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y);
		RenderText(label_pos, label);
                                 ...
                                 ImGuiWindowFlags window_flags)
{
	...
	if (bar_window == NULL || bar_window->BeginCount == 0)
	{
		...
		auto viewport = (ImGuiViewportP*)(void*)(viewport_p ? viewport_p : GetMainViewport());
...
static bool IsRootOfOpenMenuSet()
{
	...
	return (/*upper_popup->OpenParentId == window->IDStack.back() &&*/ upper_popup->Window && (upper_popup->Window->
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
	{
		...
		pressed = Selectable("", menu_is_open,
		                     ...
		                     ImGuiSelectableFlags_DontClosePopups, ImVec2(w, 0.0f));
	...
	if (menu_is_open)
	{
		...
		// Note: this is super misleading! The value will serve as reference for FindBestWindowPosForPopup(), not actual pos.
...
void ImGui::EndMenu()
{
	...
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	if (g.NavMoveDir == ImGuiDir_Left && NavMoveRequestButNoResultYet() && window->DC.LayoutType ==
		ImGuiLayoutType_Vertical)
		if (g.NavWindow && (g.NavWindow->RootWindowForNav->Flags & ImGuiWindowFlags_Popup) && g.NavWindow->
			RootWindowForNav->ParentWindow == window)
		{
			ClosePopupToLevel(g.BeginPopupStack.Size, true);
			NavMoveRequestCancel();
		}
		...
		if (selected)
			RenderCheckMark(window->DrawList,
			                pos + ImVec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f,
			                             g.FontSize * 0.134f * 0.5f), GetColorU32(ImGuiCol_Text), g.FontSize * 0.866f);
	}
	IMGUI_TEST_ENGINE_ITEM_INFO(g.LastItemData.ID, label,
                                       ...
                                       bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
	...
	bool unsaved_marker_visible = (flags & ImGuiTabItemFlags_UnsavedDocument) != 0 && (button_pos.x + button_sz <= bb.
		Max.x);
