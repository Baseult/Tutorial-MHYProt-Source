...
using ImGuiItemFlags = int; // -> enum ImGuiItemFlags_          // Flags: for PushItemFlag()
using ImGuiItemStatusFlags = int; // -> enum ImGuiItemStatusFlags_    // Flags: for DC.LastItemStatusFlags
using ImGuiOldColumnFlags = int; // -> enum ImGuiOldColumnFlags_     // Flags: for BeginColumns()
using ImGuiNavHighlightFlags = int; // -> enum ImGuiNavHighlightFlags_  // Flags: for RenderNavHighlight()
using ImGuiNavDirSourceFlags = int; // -> enum ImGuiNavDirSourceFlags_  // Flags: for GetNavInputAmount2d()
using ImGuiNavMoveFlags = int; // -> enum ImGuiNavMoveFlags_       // Flags: for navigation requests
using ImGuiNextItemDataFlags = int; // -> enum ImGuiNextItemDataFlags_  // Flags: for SetNextItemXXX() functions
using ImGuiNextWindowDataFlags = int; // -> enum ImGuiNextWindowDataFlags_// Flags: for SetNextWindowXXX() functions
using ImGuiScrollFlags = int; // -> enum ImGuiScrollFlags_        // Flags: for ScrollToItem() and navigation requests
...
struct ImGuiContext
{
	...
	ImBitArray<ImGuiKey_NamedKey_COUNT> ActiveIdUsingKeyInputMask;
...
using ImGuiTableColumnIdx = ImS8;
using ImGuiTableDrawChannelIdx = ImU8;
...
namespace ImGui
{
	...
	inline bool IsActiveIdUsingKey(ImGuiKey key)
	{
		...
		ImGuiContext& g = *GImGui;
		return g.ActiveIdUsingKeyInputMask.TestBit(key - ImGuiKey_NamedKey_BEGIN);
	}
	...
	inline void SetActiveIdUsingKey(ImGuiKey key)
	{
		IM_ASSERT(IsNamedKey(key));
		ImGuiContext& g = *GImGui;
		g.ActiveIdUsingKeyInputMask.SetBit(key - ImGuiKey_NamedKey_BEGIN);
	}
